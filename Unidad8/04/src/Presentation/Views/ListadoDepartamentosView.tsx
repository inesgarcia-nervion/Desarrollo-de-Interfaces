...
const ListadoDepartamentosView: React.FC = observer(() => {
  ...
  return (
    <div>
      ...
      <ActionHeader title="Listado de Departamentos" onAdd={() => router.push("/editarDepto")} />
      <ul>
        {vm.departamentos.map(d => (
          <li key={d._id} className="flex justify-between p-2 border-b">
            <span>{d._nombre}</span>
            <div>
              <button onClick={() => router.push(`/editarDepto?id=${d._id}`)} className="text-blue-500 mr-2">Editar</button>
